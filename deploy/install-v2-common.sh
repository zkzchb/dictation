...
# Keep the generated Caddy address list free of whitespace so the low-level
# installer can validate it before interpolating it into Caddyfile syntax.
...
step "选择外部内容包"
CONTENT_ROOT="${DICTATION_CONTENT_ROOT:-}"
[[ -n "$CONTENT_ROOT" ]] || prompt_value \
  CONTENT_ROOT "内容包绝对路径" "/opt/dictation-content/packs/zh-cn/primary-3a"
[[ "$CONTENT_ROOT" == /* ]] || die "内容包必须使用绝对路径"
[[ -f "$CONTENT_ROOT/dataset.json" ]] \
  || die "内容包不存在或缺少 dataset.json: $CONTENT_ROOT"
...
fi
{
  ...
  write_env_value CONTENT_ROOT "$CONTENT_ROOT"
  printf 'V2_PORT=8889\n'
  write_env_value APP_ROOT "$REPO_ROOT"
  printf 'STATE_ROOT=/var/lib/dictation\n'
